    ...
    public class ParkingGarage
    {
        ...
        public void ViewVehicleList()
        {
            foreach (var vehicle in _parkingSpot.Vehicles.OrderBy(v => v.Spot))
                Console.Write($"Spot Nr.: {vehicle.Spot}, Type:{vehicle.Type}, Registration Nr.:{vehicle.RegistrationNumber}, Parked Time:{vehicle.ParkedTime}\n");
        ...
        public int GetParkingSpot(string type) =>
            int.TryParse(GetInput($"Pick a parking spot for your {type} choose between 1-100: ",
                v => int.TryParse(v, out int spot) && spot > 0 && spot <= 100 && !_parkingSpot.SpotStatus(spot, type)), out int spot) ? spot : -1; //fixa
    }
}
//behöver fixas:välja spot när man väljer car, folder classen(save & read), view parked vehicle
